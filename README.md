...
When building from tarballs generated by `git-archive` or when building from a
git checkout, the current version will be encoded into the final binary. When
distributing binaries of pimsync, please ensure that `pimutils version` shows
the correct version.
